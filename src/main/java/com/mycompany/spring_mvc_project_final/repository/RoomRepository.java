...
public interface RoomRepository extends JpaRepository<RoomEntity, Long> {
    ...
    @Query("SELECT r FROM RoomEntity r WHERE r.id NOT IN(SELECT r.id FROM BookingDetailEntity bd JOIN bd.booking b JOIN bd.room r "
            + "WHERE b.checkin IN(?1,?2) OR b.checkout IN(?1,?2) "
            + "OR b.checkin = ?1 OR b.checkout = ?1 OR b.checkin = ?2 OR b.checkout = ?2) AND r.roomCategory.id = ?3")
